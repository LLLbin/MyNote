...
1. 本研究提出了一种新的基于VQGAN的超分辨率模型，在VQGAN的架构设计上添加了基于Con特征提取器，提升模型对于低分辨率图像的特征提取能力。此外，我还引入了注意力模块进行特征融合，将预训练的Encoder输出与CNN特征提取器的输出进行融合。这一设计提高了模型对低分辨率图像细节的捕捉能力，从而提升了超分辨率图像的质量。
