...
损失函数由两部分组成：
1.  $\|\operatorname{sg}[\hat{z}] - z\|_{2}^{2}$：这项损失使得从码本中选择的离散向量 $z$ 尽可能接近编码器的输出 $\hat{z}$。$\operatorname{sg}[\hat{z}]$ 表示对 $\hat{z}$ 停止梯度更新，意味着在反向传播过程中 $\hat{z}$ 的梯度不会被更新。

2.  $\beta \|\operatorname{sg}[z] - \hat{z}\|_{2}^{2}$：这项损失使得编码器的输出 $\hat{z}$ 尽可能接近从码本中选择的离散向量 $z$。$\operatorname{sg}[z]$ 表示对 $z$ 停止梯度更新，意味着在反向传播过程中 $z$ 的梯度不会被更新。这里 \$beta$ 是一个超参数，用于平衡这两项损失的权重。

### 直观解释

- **量化匹配**：第一项损失确保编码器输出的特征向量 \(\hat{z}\) 在码本中找到最接近的向量 \(z\)，并使其之间的距离最小化。这保证了特征向量 \(\hat{z}\) 能够在码本的离散向量集合中找到一个良好的表示。
  
- **编码器调整**：第二项损失通过使编码器输出的特征向量 \(\hat{z}\) 尽可能接近从码本中选择的向量 \(z\)，来调整编码器的参数。这使得编码器能够生成更加接近码本的离散向量的特征。
