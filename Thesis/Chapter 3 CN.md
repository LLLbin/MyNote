...
本文提出的超分辨率模型基于VQGAN架构，并结合了ConvNeXt特征提取模块，整个网络架构如图3-1所示，主要包括以下五个部分：编码器（Encoder）、ConvNeXt特征提取器、码本（Codebook）、解码器（Decoder）和判别器（Discriminator）。在网络架构中，首先LR图像$x$ 会进入Encoder，Encoder会对其进行压缩与编码。而后输出的结果 $\hat{x}$ 会进入ConvNeXt模块中进行深度的提取特征。之后，提取到的连续特征 $\hat{z}$ 会在Codebook中被量化为离散形式 $z_{q}$ ，再由Decoder将量化后的特征表示 $z_{q}$ 重建为高分辨率的图像。最后由Discriminator对SR图像和HR图像进行判别，通过对抗性训练提升了生成图像的逼真度和细节还原能力。
#### Encoder
...
我们的方法整体训练分为两个阶段。在第一阶段，我们主要训练VQGAN的骨架网络，包括编码器、码本和解码器，用以提取高分辨率图像中的先验信息，为第二阶段的超分辨任务提供基础。在第二阶段，我们结合第一阶段的固定码本和固定解码器，训练编码器、ConvNeXt特征提取器以及判别器，以解决图像盲超分辨率任务。
