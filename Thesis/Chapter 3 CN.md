...
本文提出的超分辨率模型基于VQGAN架构，并结合了ConvNeXt特征提取模块，整个网络架构如图3-1所示，主要包括以下五个部分：编码器（Encoder）、ConvNeXt特征提取器、码本（Codebook）、解码器（Decoder）和判别器（Discriminator）。我们的方法整体训练分为两个阶段。在第一阶段，我们主要训练VQGAN的骨架网络，包括编码器、码本和解码器，用以提取高分辨率图像中的先验信息，为第二阶段的超分辨任务提供基础。在第二阶段，我们结合第一阶段的固定码本和固定解码器，训练编码器、ConvNeXt特征提取器以及判别器，以解决图像盲超分辨率任务。
#### Encoder
