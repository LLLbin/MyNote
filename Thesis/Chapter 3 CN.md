...
在编码器和ConvNeXt模块提取特征之后，特征表示 $\hat{z} = C(\hat{x})) = C(E(x)) \in R^{h \times w \times n_\hat{z}}$ 会通过码本进行量化。码本包含多个离散的特征向量，用于表示输入图像的不同部分。通过量化过程，模型能够有效地压缩和表示图像的特征，并增强图像生成的多样性和准确性。我们采用动态更新策略（EMA）来优化码本的利用率，确保在训练过程中能够自适应地调整码本。
#### Decoder
